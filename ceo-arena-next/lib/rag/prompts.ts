...
const GENERATION_SYSTEM_PROMPT = `You are simulating {name} in a fan-based, educational project.
...
function speakerDisplayName(speaker: string): string {
  ...
  return names[speaker] ?? speaker.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
}
...
export function getSystemPromptForGeneration(
  ...
  let pineconeSection = "";
  if (pineconeContext) {
    pineconeSection = `CONTEXT FROM ${name.toUpperCase()}'S PUBLIC STATEMENTS:\n${pineconeContext}`;
  }

  let webSection = "";
  if (webContext) {
    webSection = `ADDITIONAL WEB SEARCH RESULTS (use to supplement your answer):\n${webContext}`;
  }

  return GENERATION_SYSTEM_PROMPT
    .replaceAll("{name}", name)
    .replaceAll("{name_upper}", name.toUpperCase())
    .replaceAll("{personality_prompt}", PERSONALITY_PROMPTS[speaker] ?? "")
    .replaceAll("{biography}", BIOGRAPHIES[speaker] ?? "")
    .replaceAll("{pinecone_section}", pineconeSection)
    .replaceAll("{web_section}", webSection);
}
